...
function updateSettings() {
  ...
  settings.clothesColor = document.getElementsByName('shirts')[0].checked ? settings.color : 'default';
...
function drawMii() {
  ...
  mii.addEventListener('load', function () {
...
for (let es = 0; es < document.getElementsByTagName('select').length; es++) {
  document.getElementsByTagName('select')[es].addEventListener('change', updateImage);
}
for (let es = 0; es < document.querySelectorAll('input[type=checkbox]').length; es++) {
  document.querySelectorAll('input[type=checkbox]')[es].addEventListener('change', updateImage);
}
        ...
        if (ctx.measureText(string).width > 496) {
          if (lastSpacePos === -1) {
            if (lines[i + 1] === undefined) {
              lines[i + 1] = lines[i].slice(j + 1);
            } else {
              lines[i + 1] = lines[i].slice(j + 1) + lines[i + 1];
            }
            lines[i] = string.slice(0, Math.max(0, j + 1));
          } else {
            if (lines[i + 1] === undefined) {
              lines[i + 1] = lines[i].slice(lastSpacePos + 1);
            } else {
              lines[i + 1] = lines[i].slice(lastSpacePos + 1) + lines[i + 1];
            }
            lines[i] = lines[i].slice(0, Math.max(0, lastSpacePos + 1));
          }
        }
      }
    }
...
document.getElementsByName('id')[0].addEventListener('input', updateImage);
document.getElementById('to-input').addEventListener('input', updateSettings);
document.getElementById('from-input').addEventListener('input', updateSettings);
